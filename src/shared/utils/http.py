"""Lightweight HTTP helpers for attaching `FLOWMESH_API_KEY` to outgoing requests.

Kept dependency-free (`os` only) so callers that don't need `aiohttp` / `requests`
— in particular the worker `ssh_executor` import chain under
`runtime-worker-core` — can import it without pulling either in.
"""
