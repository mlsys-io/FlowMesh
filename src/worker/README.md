...
| `WORKER_ALIAS` | token name | The worker's name. The supervisor sets it for workers it launches; an external worker defaults to the name in its token. |
...
4. Downstream stages reference the URL via `${stage.final_model_archive}`
