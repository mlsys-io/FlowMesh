...
| `NODE_ALIAS` | `node` | Node name; must be unique among live nodes, so set a distinct value on every node of a multi-node stack. A node whose alias another live node holds is refused registration |
