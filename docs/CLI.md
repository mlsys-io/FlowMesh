...
flowmesh worker   {list, info, cordon, uncordon, cordons}
...
Preset workers (`cpu` / `gpu`) take their aliases from the stack slug, so they
are isolated across stacks sharing a host the same way other stack objects are:
`flowmesh_node_worker_cpu_0` by default, `flowmesh_node_<suffix>_worker_cpu_0`
when `FLOWMESH_STACK_SUFFIX` is set. Override the aliases with
`--alias-template`, which accepts the placeholders `{slug}`, `{kind}`, `{idx}`,
and `{gpu}`:

```bash
flowmesh stack worker up cpu 2 --alias-template '{slug}-run-{idx}'  # {slug}-run-0, {slug}-run-1
```

The template must keep aliases unique within one `up` invocation — include
`{idx}` or `{gpu}` when creating more than one worker. `flowmesh stack worker
{start,stop,down}` and `flowmesh node worker {start,stop}` take worker aliases.
...
With one or more `SERVICE` arguments (`server`, `redis_control`,
