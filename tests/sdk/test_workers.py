...
def _cpu_payloads(
    count: int, slug: str | None = None, alias_template: str | None = None
) -> list[tuple[str, str]]:
...
def test_template_renders_placeholders() -> None:
    payloads = _cpu_payloads(2, slug="flowmesh_node", alias_template="{slug}-run-{idx}")
    assert _aliases(payloads) == ["flowmesh_node-run-0", "flowmesh_node-run-1"]


def test_template_kind_placeholder() -> None:
    payloads = _cpu_payloads(1, slug="s", alias_template="{kind}-{idx}")
...
def test_unknown_placeholder_raises_with_available_fields() -> None:
    with pytest.raises(FlowMeshError) as exc:
        _cpu_payloads(1, alias_template="{bogus}")
...
def test_malformed_template_raises_flowmesh_error() -> None:
    with pytest.raises(FlowMeshError, match="invalid --alias-template"):
        _cpu_payloads(1, alias_template="{slug")


def test_template_missing_index_collides() -> None:
    with pytest.raises(FlowMeshError, match="duplicate worker aliases"):
        _cpu_payloads(2, alias_template="{slug}-static")
