"""Append the FlowMesh image digest table to a GitHub Release body.

Reads the current Release body via ``gh release view``, strips any block
bounded by the FlowMesh sentinel comments, then appends the table from
...
START_MARKER = "<!-- flowmesh-image-digests:start -->"
END_MARKER = "<!-- flowmesh-image-digests:end -->"
...
def _read_current_body(gh: str, tag: str) -> str:
    result = (
        subprocess.run(  # nosec B603: argv list, no shell, gh path from shutil.which()
...
def _write_release_body(gh: str, tag: str, body: str) -> None:
    with tempfile.NamedTemporaryFile(
        ...
        notes_path = f.name
    try:
        result = subprocess.run(  # nosec B603: argv list, no shell, gh path from shutil.which()
