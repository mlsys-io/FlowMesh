...
Every `setup-uv@v7` invocation in CI must pin its `version:` to the same `UV_VERSION` the image was built with, and must keep `enable-cache` off (the runner image already supplies the cache; GHA cache would override `~/.cache/uv` with whatever stale state lives in the GHA cache backend):

```yaml
- uses: astral-sh/setup-uv@94527f2e458b27549849d47d273a16bec83a01e9  # v7
  with:
    version: "0.11.8"        # must match Dockerfile's UV_VERSION
```

(`enable-cache` defaults to `auto` on self-hosted, which is off — leave it unset.)
